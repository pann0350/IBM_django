...
def show_exam_result(request, course_id, submission_id):
    ...
    for question in course.question_set.all():
        ...
        q_ctx = {'question_text':question.question_text, 'choices':[]}
        
        for choice in question.choice_set.all():
            if choice.id in selected_choices_ids:
                if choice.is_correct:
                    q_ctx['choices'].append(f'<p style="color:green">Correct answer: {choice.choice_text}</p>')
                else:
                    q_ctx['choices'].append(f'<p style="color:red">Wrong answer: {choice.choice_text}</p>')
                    add_score = False
            else:
                if choice.is_correct:
                    q_ctx['choices'].append(f'<p style="color:orange">Not selected: {choice.choice_text}</p>')
                    add_score = False
                else:
                    q_ctx['choices'].append(f'<p>{choice.choice_text}</p>')
